...
void onAlarm(void) 
{
	...
	Error = 0;	
    for(int i=0;i<8;i++){
		temperature[i].alarm = ((temperature[i].PV10 > temperature[i].SV10)||(temperature[i].PV10 < temperature[i].SV01)) ? 1 : 0;
		if(temperature[i].alarm) {
			if(temperature[i].on_off)
			Error = 1; 
		}
	}
    if(Error) {
        GPIOC->ODR ^= GPIO_Pin_10|GPIO_Pin_13;  
        GPIO_SetBits(GPIOC,GPIO_Pin_14);
    }
	if((bit)&&(!Error)) {
        GPIO_ResetBits(GPIOC,GPIO_Pin_10|GPIO_Pin_13);
